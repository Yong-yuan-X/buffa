...
The committed history (PR #211) is the v1 dataset: built at the pinned toolchain and profile with median-of-N, but with each shape's series starting at the release that introduced it to the suite, and the two message-set-change transitions (v0.3.0→v0.4.0 added `MediaFrame`, v0.7.0→v0.7.1 added `PackedTile`) annotated as inliner-coupling artifacts rather than code changes. This design replaces that with the dense, fully isolated matrix described above.
