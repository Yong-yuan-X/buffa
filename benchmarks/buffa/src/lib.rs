...
//!
//! v0.3.0 buffa-codegen emits one file per input proto (named by file stem)
//! and has no `include_proto!` macro, so each per-message build includes only
//! the single iso proto whose feature is enabled, keeping the codegen unit free
//! of every other message shape's decoder.
